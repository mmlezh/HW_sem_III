...
void initialization_of_main_structures(struct point *min, struct point *max, struct point *base_dot, struct point *main_dot, struct line *cross_line, struct line *edges);
...
int main(int argc, char *argv[argc])
{
	...
	initialization_of_main_structures(&min, &max, &base_dot, &main_dot, &cross_line, edges);
...
int do_the_lines_cross(struct line *line, struct line *cross_line, struct point *cross_point)
{
	...
	double temp_min_x, temp_min_y;
	double temp_max_x, temp_max_y;
	...
	temp_max_y = (line->p1->y > line->p2->y) ? line->p1->y : line->p2->y;
	temp_min_y = (line->p1->y < line->p2->y) ? line->p1->y : line->p2->y;
	...
	if(det)
	{
		...
		if((temp_max_x >= det_x/det) && (temp_min_x <= det_x/det) && (temp_max_y >= det_y/det) && (temp_min_y <= det_y/det))
		{
...
int num_of_intersections(struct line *edges, struct line *cross_line, int size)
{
	...
	for(int i = 0; i < size; i++)
	{
		...
		if((flag == CROSS) && ((fabs(temp_cross_point.x - edges[i].p2->x) > EPS) || (fabs(temp_cross_point.y - edges[i].p2->y) > EPS)))
		{
			...
			{
				//printf("\n\n %.2lf  %.2lf\n\n", temp_cross_point.x , temp_cross_point.y );
				cross_num++;
			}
		}
	}
...
void initialization_of_main_structures(struct point *min, struct point *max, struct point *base_dot, struct point *main_dot, struct line *cross_line, struct line *edges)
{
	...
	*/
	...
	base_dot->x = (edges[0].p1->x + edges[0].p2->x)/2;	
	base_dot->y = (edges[0].p1->y + edges[0].p2->y)/2;

	if((fabs(base_dot->x - main_dot->x) < EPS) && (fabs(base_dot->y - main_dot->y) < EPS))
	{
		base_dot->x = (edges[1].p1->x + edges[1].p2->x)/2;	
		base_dot->y = (edges[1].p1->y + edges[1].p2->y)/2;
	}
...
int check(struct line *edges, struct point *main_dot, struct line *cross_line, struct point *min, struct point *max, int size)
{
	...
	for(int i = 0; i < size; i++) 
	{
		...
		temp_max_y = (edges[i].p1->y > edges[i].p2->y) ? edges[i].p1->y : edges[i].p2->y;
		temp_min_y = (edges[i].p1->y < edges[i].p2->y) ? edges[i].p1->y : edges[i].p2->y;
 		
 		if(fabs(edges[i].A * main_dot->x + edges[i].B * main_dot->y + edges[i].C) < EPS)//принадлежит ли точка прямой?
 		{
			if((main_dot->x >= temp_min_x) && (main_dot->x <= temp_max_x) && (main_dot->y >= temp_min_y) && (main_dot->y <= temp_max_y))//проверка навыход из рамки
			{
				return ON_POLY; 
			}
		} 
